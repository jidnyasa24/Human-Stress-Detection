...
def app(df, x, y):
    ...
    st.title("Depression Detection Page")

    # Add a brief description
    st.markdown(
        """
            <p style="font-size:25px">
                This app uses <b style="color:green">Decision Tree Classifier</b> for the Detection of Depression Level.
            </p>
    ...
    if st.button("Detect"):
        ...
        st.info("Depression level detected...")

        # Print the output according to the detection
        if detection == 1:
            st.success("The person has low Depression level 🙂")
        elif detection == 2:
            st.warning("The person has medium Depression level 😐")
        elif detection == 3:
            st.error("The person has high Depression level! 😞")
        elif detection == 4:
            st.error("The person has very high Depression level!! 😫")
        else:
            st.success("The person is Depression free and calm 😄")
